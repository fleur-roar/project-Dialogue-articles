...
def obrabotchic(tex):
    ...
    var_final = 0
    count_in = 0
    links_2_articles = {'название 1 статьи': 'https://drive.google.com/drive/folders/1VkJC9bR2zyyrSEclgQcyroyxPmwOSUUg?usp=sharing', '2я статья': '2я ссылка', '3я статья': '3я ссылка и тд.'}
...
def handle_text(message):
    if message.text == 'Обработка статьи 🤍':
        ...
        bot.send_message(message.chat.id, 'Уупс.. похоже, этот раздел ещё не доделан, приходите позже!🌾  ', reply_markup=source_language_menu)

    elif message.text == 'Достать статью Диалога ❤':
        B_menu = types.ReplyKeyboardMarkup(True, True)
        B_menu.row('Назад')
        bot.send_message(message.chat.id, 'Напишите, пожалуйста, название статьи вот в таком формате🌾 \n \nстатья: название статьи', reply_markup=B_menu)

    elif message.text == 'Интересные факты 🤍':
        C_menu = types.ReplyKeyboardMarkup(True, True)
        C_menu.row('Авторы', 'Key-words')
        C_menu.row('Назад')
        bot.send_message(message.chat.id, 'Уупс.. похоже, этот раздел ещё не доделан, приходите позже!🌾 ', reply_markup=C_menu)
        ...
        bot.send_message(message.chat.id, 'Прости, я не понимаю, попробуй начать с /menu')
